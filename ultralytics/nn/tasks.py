...
class BaseModel(nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        """
        Perform a forward pass through the network once (no augmentations).
        This is used for both training and inference.

        Args:
            x (torch.Tensor): Input tensor.
            profile (bool): Whether to profile each layer.
            visualize (str): Directory path to save feature visualizations.
            embed (list[int]): List of layer indices from which to extract embeddings.

        Returns:
            torch.Tensor: The final output tensor of the network.
        """
        y, dt, embeddings = [], [], []  # outputs, profile times, embeddings
        ...
        for m in self.model:
            ...
            # profile 计算 FLOPs 和时间
            if profile:
                self._profile_one_layer(m, x_in, dt)

            if isinstance(m, AGLE) and isinstance(x_in, list) and len(x_in) == 2:
                x = m(x_in[0], x_in[1])
            else:
                x = m(x_in)
